...
type detector struct {
	// annotationPrefix is the container annotation prefix, e.g.
	// "nvidia.com/container." — must match the sharingd plugin's
	// configured prefix.
...
func (d detector) check(pod *api.PodSandbox, container *api.Container) (violator, bool) {
	...
	visibleDevices := annotations.ParseVisibleDevices(
		pod.GetAnnotations(), container.GetName(), d.annotationPrefix)
