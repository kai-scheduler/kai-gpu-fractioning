// Package gpusharingconfig provides typed CRUD and status helpers for the
// cluster-scoped GpuSharingConfig custom resource, so the operator e2e suite can
...
// scheme.Scheme (core + apps only), so this package registers the gpu-sharing
...
func init() {
	// Register GpuSharingConfig into the global scheme the cluster client uses,
...
// DefaultName is the only name the CRD's CEL rule accepts for a GpuSharingConfig.
const DefaultName = "default"

// Get returns the default GpuSharingConfig.
func Get(ctx context.Context, c *cluster.Client) (*v1alpha1.GpuSharingConfig, error) {
	return getByName(ctx, c, DefaultName)
}

// getByName returns the named GpuSharingConfig (cluster-scoped). The CRD's CEL
// rule allows only "default", so this stays unexported — callers use Get.
func getByName(ctx context.Context, c *cluster.Client, name string) (*v1alpha1.GpuSharingConfig, error) {
	var out v1alpha1.GpuSharingConfig
...
// Create creates the given GpuSharingConfig object.
func Create(ctx context.Context, c *cluster.Client, obj *v1alpha1.GpuSharingConfig) error {
...
// the live default CR.
func CreateDryRun(ctx context.Context, c *cluster.Client, obj *v1alpha1.GpuSharingConfig) error {
	return c.Ctrl.Create(ctx, obj, ctrlclient.DryRunAll)
}

// CreateUnstructuredDryRun dry-run-creates an arbitrary (possibly invalid)
// GpuSharingConfig expressed as a spec map. It exists for schema-negative cases
...
func CreateUnstructuredDryRun(ctx context.Context, c *cluster.Client, name string, spec map[string]any) error {
	...
	u.SetGroupVersionKind(v1alpha1.SchemeGroupVersion.WithKind("GpuSharingConfig"))
	u.SetName(name)
...
func Patch(ctx context.Context, c *cluster.Client, name string, mergePatch []byte) error {
	obj := &v1alpha1.GpuSharingConfig{ObjectMeta: metav1.ObjectMeta{Name: name}}
...
func WaitGone(ctx context.Context, c *cluster.Client, name string, timeout, interval time.Duration) error {
	return waiter.PollUntil(ctx, timeout, interval, fmt.Sprintf("GpuSharingConfig/%s to be deleted", name),
...
func Condition(obj *v1alpha1.GpuSharingConfig, condType string) (metav1.Condition, bool) {
...
func WaitCondition(ctx context.Context, c *cluster.Client, name, condType string, status metav1.ConditionStatus, timeout, interval time.Duration) (*v1alpha1.GpuSharingConfig, error) {
	var latest *v1alpha1.GpuSharingConfig
	err := waiter.PollUntil(ctx, timeout, interval,
		fmt.Sprintf("GpuSharingConfig/%s condition %s=%s", name, condType, status),
		func(ctx context.Context) (bool, error) {
